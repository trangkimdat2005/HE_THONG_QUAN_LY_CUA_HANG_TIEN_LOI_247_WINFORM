...
namespace HE_THONG_QUAN_LY_CUA_HANG_TIEN_LOI_247_WINFORM.PresentationLayer.Forms.Products
{
    public partial class frmMeasurements : Form
    {
        ...
        private void btnExport_Click(object sender, EventArgs e) => ExportToExcel(dgvUnits);
        ...
        private void ExportToExcel(DataGridView dgv)
        {
            ...
            try
            {
                using (SaveFileDialog sfd = new SaveFileDialog())
                {
                    ...
                    sfd.FileName = "DanhSachDonViDoLuong_" + DateTime.Now.ToString("ddMMyy_Hmmss") + ".xlsx";

                    if (sfd.ShowDialog() == DialogResult.OK)
                    {
                        using (var workbook = new ClosedXML.Excel.XLWorkbook())
                        {
                            var worksheet = workbook.Worksheets.Add("Đơn Vị Đo Lường");
