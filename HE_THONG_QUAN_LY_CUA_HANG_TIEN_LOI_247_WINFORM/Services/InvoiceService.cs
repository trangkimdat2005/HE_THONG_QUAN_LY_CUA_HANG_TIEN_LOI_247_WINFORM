        ...
        public Tuple<bool, string, string> CreateInvoice(HoaDon invoice, List<ChiTietHoaDon> details)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    invoice.trangThai = "Ch? thanh toán";
                    ...
                    return Tuple.Create(true, "T?o hóa ??n thành công", invoice.id);
                }
                catch (Exception ex)
                {
            ...
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (existingInvoice == null)
                        return Tuple.Create(false, "Không tìm th?y hóa ??n");
                    ...
                    return Tuple.Create(true, "C?p nh?t hóa ??n thành công");
                }
                catch (Exception ex)
                {
                    transaction.Rollback();
        ...
        public Tuple<bool, string> DeleteInvoice(string id)
        {
            try
            {
                ...
                if (invoice.trangThai == "?ã thanh toán")
                    return Tuple.Create(false, "Không th? xóa hóa ??n ?ã thanh toán");
                ...
                _context.SaveChanges();
                return Tuple.Create(true, "Xóa hóa ??n thành công");
            }
            catch (Exception ex)
            {
                return Tuple.Create(false, $"L?i: {ex.Message}");
        ...
        public Tuple<bool, string> ProcessPayment(string invoiceId, string paymentMethodId, decimal amount, string description)
        {
            using (var transaction = _context.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (invoice.trangThai == "?ã thanh toán")
                        return Tuple.Create(false, "Hóa ??n ?ã ???c thanh toán");
                    ...
                    {
                        invoice.trangThai = "?ã thanh toán";
                    }
                    else
                    {
                        invoice.trangThai = "?ang thanh toán";
                    }
