...
namespace HE_THONG_QUAN_LY_CUA_HANG_TIEN_LOI_247_WINFORM.BLL.Services
{
    public class GoodsService
    {
        ...
        public string GenerateNewLinkID()
        {
            try
            {
                var allIds = _context.SanPhamDanhMucs
                    .AsNoTracking()
                    .Where(x => x.id.StartsWith("SPDM"))
                    .Select(x => x.id)
                    .ToList();

                if (allIds.Count == 0) return "SPDM0001";

                int maxNum = 0;
                foreach (var id in allIds)
                {
                    if (id.Length > 4 && int.TryParse(id.Substring(4), out int num))
                    {
                        if (num > maxNum) maxNum = num;
                    }
                }

                return $"SPDM{(maxNum + 1):D4}";
            }
            catch
            {
                return $"SPDM{Guid.NewGuid().ToString().Substring(0, 4).ToUpper()}";
            }
        }
